...
function extractFeatures(directory, salida, dataset)
    ...
    for i in 1:length(dir)
        ...
        nrows = length(imagen[:, 1])
        ncols = length(imagen[1, :])
        
        nrows25 = convert(Int, round(nrows / 4))
        nrows50 = convert(Int, round(nrows25 * 2))
        nrows75 = convert(Int, round(nrows25 * 3))
        ncols25 = convert(Int, round(ncols / 4))
        ncols75 = convert(Int, round(ncols25 * 3))

        imagenRecortada = imagen[nrows50:nrows75, ncols25:ncols75]
        ...
        imagenObjetosR = RGB.(matrizBooleanaR, matrizBooleanaR, matrizBooleanaR)
        ...
        write(file, ",")
        write(file, string(sum(convert(Array{Int64, 2}, matrizBooleanaR))))
        write(file, ",")
        write(file, string(sum(convert(Array{Int64, 2}, matrizBooleanaG))))
        write(file, ",")
        write(file, string(sum(convert(Array{Int64, 2}, matrizBooleanaB))))
        write(file, ",")
        write(file, string(sum(convert(Array{Int64, 2}, ImageMorphology.diameters(MaxTree(canalR .> 0.5, connectivity=2))))))
        write(file, ",")
        write(file, string(sum(convert(Array{Int64, 2}, ImageMorphology.diameters(MaxTree(canalG .> 0.5, connectivity=2))))))
        write(file, ",")
        write(file, string(sum(convert(Array{Int64, 2}, ImageMorphology.diameters(MaxTree(canalB .> 0.5, connectivity=2))))))
...
extractFeatures("./datasets/mascarilla/", 1, "./datasets/aprox6/patrones.data")
extractFeatures("./datasets/no_mascarilla/", 0, "./datasets/aprox6/patrones.data")

dataset = readdlm("./datasets/aprox6/patrones.data", ',')
