...
indexArray = crossvalidation(targets, numFolds)
# trainingIndex, validationIndex = holdOut(size(trainingInputs, 1), validationRatio)
...
for numFold in 1:numFolds
    ...
    trainingInputs = inputs[indexArray .!= numFold, :]
    testInputs = inputs[indexArray .== numFold, :]
    trainingTargets = targets[indexArray .!= numFold, :]
    testTargets = targets[indexArray .== numFold, :]

    testAccuraciesPerRepetition = Array{Float64, 1}(undef, trainingRepetition)
    testF1PerRepetition = Array{Float64, 1}(undef, trainingRepetition)

    for numTraining in 1:trainingRepetition
        if validationRatio > 0
            local trainingIndex, validationIndex
            trainingIndex, validationIndex = holdOut(size(trainingInputs, 1), validationRatio)
            
            # local ann
            ann, _ = train(topology, (trainingInputs[trainingIndex, :], trainingTargets[trainingIndex, :]), maxEpochs, minLoss, learningRate, maxEpochsVal, (trainingInputs[validationIndex, :], trainingTargets[validationIndex, :]), (testInputs, testTargets))

        else
            #local ann
            ann, _ = train(topology, (trainingInputs, trainingTargets), maxEpochs=maxEpochs, learningRate=learningRate, testSet=(testInputs, testTargets))
        end
        acc, _, _, _, _, _, F1, _ = confusionMatrix(collect(ann(testInputs')'), testTargets, true)

        testAccuraciesPerRepetition[numTraining] = acc
        testF1PerRepetition[numTraining] = F1
    end

    testAccuracies[numFold] = mean(testAccuraciesPerRepetition)
    testF1[numFold] = mean(testF1PerRepetition)

    println("Results in test in fold ", numFold, "/", numFolds, " : accuracy: ", 100*testAccuracies[numFold], " %, F1: ", 100*testF1[numFold], " %")

end

println("Average test accuracy on a ", numFolds, "-fold crossvalidation: ", 100*mean(testAccuracies), " , with a standard derivation of ", 100*std(testAccuracies))
println("Average test F1 on a ", numFolds, "-fold crossvalidation: ", 100*mean(testF1), " , with a standard derivation of ", 100*std(testF1))
