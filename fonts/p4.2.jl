...
function train(topology::AbstractArray{<:Int,1}, dataset::Tuple{AbstractArray{<:Real,2},AbstractArray{Bool,2}}, testSet::Tuple{AbstractArray{<:Real,2}, AbstractArray{Bool,2}},
     validationSet::Tuple{AbstractArray{<:Real,2}, AbstractArray{Bool,2}};
    maxEpochs::Int=1000, minLoss::Real=0, learningRate::Real=0.01, maxEpochsVal::Int=20)

    println("OWo")
    trainingInputs = dataset[1]' 
    trainingTargets = dataset[2]'

    testInputs = testSet[1]'
    testTargets = testSet[2]'

    # println("Training targets: ", trainingTargets)
    # println("Test targets: ",testTargets)

    if (isassigned(validationSet[1], 2))    #Se comprueba que el set de Validacion no es nulo
        validationInputs = validationSet[1]'
        validationTargets = validationSet[2]'
        # println("Validation targets: ", validationTargets)
    ...
    function calculateMetrics()
        ...
        @assert(minimum(trainingInputs) >= 0)
        @assert(minimum(testInputs) >= 0)
        @assert(minimum(validationInputs) >= 0)
...
(trainingIndex, testIndex) = holdOut(size(inputs, 1), testRatio)
#(trainingIndex, validationIndex, testIndex) = holdOut(size(inputs,1), validationRatio, testRatio);

trainingInputs    = inputs[trainingIndex,:]; 
validationInputs  = inputs[validationIndex,:]; 
testInputs        = inputs[testIndex,:]; 
trainingTargets   = targets[trainingIndex,:]; 
...
for numClass in 1:numClasses
    #ann, = train(topology, (trainingInputs, trainingTargets[:,[numClass]]); validationSet=(validationInputs, validationTargets[:,[numClass]]), testSet=(testInputs, testTargets[:,[numClass]]));
    ann, = train(topology, (trainingInputs, trainingTargets[:,[numClass]]), (testInputs, testTargets[:,[numClass]]), (AbstractArray{<:Real,2}, AbstractArray{Bool,2}));
    outputs[:, numClass] = ann(inputs')'
end

outputs = collect(softmax(outputs')');
println("Results in the training set:")
printConfusionMatrix(outputs[trainingIndex,:], trainingTargets, true);
println("Results in the validation set:")
printConfusionMatrix(outputs[validationIndex,:], validationTargets, true);
println("Results in the test set:")
printConfusionMatrix(outputs[testIndex,:], testTargets, true);
